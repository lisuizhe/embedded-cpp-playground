...
const char* kShareMemPath = "/sample_point";
...
public:
  SharedMem(const char* name, bool owner=false) {
    ...
    if (fd == -1) {
      throw std::runtime_error("Failed to open a shared memery region");
    }
    if (ftruncate(fd, sizeof(T)) < 0) {
      close(fd);
      throw std::runtime_error("Failed to set size of a shared memory rigion");
    }
...
void producer() {
  SharedMem<Payload> writer(kShareMemPath);
...
void consumer() {
  SharedMem<Payload> point_reader(kShareMemPath, true);
